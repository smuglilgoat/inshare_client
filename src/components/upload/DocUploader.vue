<template>
  <div>
    ...
    <!--SUCCESS-->
    <DocViewer :doc="document" v-if="document.path" class="mb-2"/>
  ...
  methods: {
    ...
    save(formData) {
      ...
      this.upload(formData)
        .then(x => {
          this.document = x.data.ddocument;
          axios
            .get("/documents/" + this.document.id + "/doc")
            .then(data => (this.document.path = data.data.doc.path));
    ...
    upload(formData) {
      ...
      return axios.post("/documents/create/t=doc", formData, {
        headers: {
